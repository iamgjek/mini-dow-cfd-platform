...
`TFFITMQ+` 這類代碼是固定月份的合約,本身不會像 `TFFIMTX8`/`TFFITM1` 那樣自動轉倉——但這個平台會**自動幫它轉倉**,見下方「合約轉倉」。

要換成別的商品系列(例如改追蹤大台指或小台指),改環境變數 `TF_SYMBOL` 並同步調整 `backend/config.py` 的 `CONTRACT_MULTIPLIER` 即可;這麼做之後自動轉倉邏輯就不適用了(它假設固定追蹤「微小台指」系列)。

## 合約轉倉

這個平台固定追蹤微小台指的近月合約,並依照下面的規則自動轉倉(邏輯在 `backend/contract_calendar.py`,由 `backend/rollover.py` 在背景排程執行,不需要手動介入,也不需要重啟服務):

- **每月第三個星期三的前一天 15:45(台北時間)**,系統會:
  1. 把當時所有還有部位的會員,依市價**強制全部平倉**(最多重試 3 次,若行情當下真的抓不到有效價格會拒單並在後端 log 留下警告,需要人工檢查)
  2. 把追蹤的合約代碼換成下個月份(例如 8 月合約 `TFFITMQ+` 換成 9 月合約 `TFFITMU+`),同時清空報價歷史(不同合約的價格不能混在同一張圖表裡)
  3. 用新的 `instrument` WebSocket 訊息廣播給所有連線中的前端,畫面上的商品名稱/代碼、報價、圖表會即時更新,不用重新整理頁面
- 以 2026 年 8 月為例:8/19 是當月第三個星期三,所以 **8/18 15:45** 觸發轉倉,轉倉後追蹤 9 月合約,8/19 起交易的就是九月合約
- 伺服器啟動時 `TF_SYMBOL` 的預設值也是用同一套規則現算的(見 `config.py`),所以就算服務重新部署,一開機用的合約代碼也一定是對的月份
